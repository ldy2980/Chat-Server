...
import { AlertCircle, CheckCircle, X, LogOut } from "lucide-react";
...
function App() {
  ...
  const saveSelectedRoomToStorage = useCallback(
    (room: ChatRoom | null) => {
      try {
        if (room && currentUser) {
          localStorage.setItem(
            STORAGE_KEYS.SELECTED_ROOM,
            JSON.stringify(room),
          );
        } else {
          localStorage.removeItem(STORAGE_KEYS.SELECTED_ROOM);
    ...
    try {
      ...
      setConsecutiveErrors((prev) => {
        ...
        const newInterval = Math.min(
          30000 * Math.pow(1.5, newErrorCount),
          300000,
        );
      ...
      setLastErrorTime((prev) => {
        ...
        if (now - prev > 30000) {
          addNotification({
            ...
            title: "서버 연결 오류",
            message:
              "서버에 연결할 수 없습니다. 잠시 후 자동으로 재시도됩니다.",
            timestamp: now,
            read: false,
          });
          return now;
          ...
          {selectedChatRoom ? (
            ...
            <div style={welcomeStyle} className="animate-fadeIn">
              ...
              <p style={welcomeDescriptionStyle}>
                왼쪽에서 채팅방을 선택하거나 새로운 채팅방을 만들어서 실시간
                대화를 시작할 수 있습니다.
              </p>
