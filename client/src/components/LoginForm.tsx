...
const LoginForm: React.FC<LoginFormProps> = ({ onLogin, onError }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onError(
        error.response?.data?.message || error.message || "인증에 실패했습니다",
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div style={containerStyle} className="animate-fadeIn">
      <div style={headerStyle}>
        ...
        <h1 style={titleStyle}>{isLogin ? "로그인" : "회원가입"}</h1>
      ...
      <div style={switchModeStyle}>
        ...
        <button type="button" onClick={toggleMode} style={switchLinkStyle}>
          {isLogin ? "회원가입" : "로그인"}
        </button>
      </div>
    </div>
