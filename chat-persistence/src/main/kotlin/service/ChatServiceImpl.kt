...
//            throw IllegalArgumentException("채팅방이 가득 찼습니다.")
//        }
        ...
        if(webSocketSessionManager.isUserOnlineLocally(user.id)) {
            webSocketSessionManager.joinRoom(user.id, roomId)
        }
    }
    ...
    override fun sendMessage(
        ...
        try {
            redisMessageBroker.broadcastToRoom(
                ...
                message = chatMessage,
                excludeServerId = redisMessageBroker.getSeverId()
            )
        } catch (e: Exception) {
